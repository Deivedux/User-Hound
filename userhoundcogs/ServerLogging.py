...
class ServerLogging:
	...
	async def on_message_edit(self, before, after):
		...
		if after.guild.id in logging_channels.keys():
			...
			if channel != None:
				...
				embed.set_footer(text = str(after.author), icon_url = after.author.avatar_url)
	...
	async def on_member_join(self, member):
		if member.guild.id in logging_channels.keys():
			...
			if channel != None:
				embed = discord.Embed(title = '<:memberjoin:439643745089224704> Member Joined', description = 'There are now ' + str(member.guild.member_count) + ' members.', color = 0x28B463, timestamp = datetime.datetime.utcnow())
	...
	async def on_member_remove(self, member):
		if member.guild.id in logging_channels.keys():
			channel = self.bot.get_channel(logging_channels[member.guild.id])
			if channel != None:
				embed = discord.Embed(title = '<:memberleave:439643745059733514> Member Left', description = 'There are now ' + str(member.guild.member_count) + ' members.', color = 0xCB4335, timestamp = datetime.datetime.utcnow())
				embed.set_footer(text = str(member), icon_url = member.avatar_url)
				await channel.send(embed = embed)
			else:
				c.execute("DELETE FROM ServerLogging WHERE Guild = " + str(member.guild.id))
